...
public class CurlView extends GLSurfaceView implements View.OnTouchListener,
	...
	private OnPageAnimationDoneListener onPageAnimationDoneListener;
	...
	public void setOnPageAnimationDoneListener(OnPageAnimationDoneListener onPageAnimationDoneListener){
		this.onPageAnimationDoneListener = onPageAnimationDoneListener;
	}
	...
	public void onDrawFrame() {
		...
		if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
			...
			mAnimate = false;
			requestRender();
			if(onPageAnimationDoneListener!=null){
				onPageAnimationDoneListener.onPageAnimationDone(mCurrentIndex);
			}
	...
	public interface OnPageAnimationDoneListener{
		public void onBeforeAnimationDone(int index);
		public void onPageAnimationDone(int index);
	...
	public boolean onTouch(View view, MotionEvent me) {
		...
		case MotionEvent.ACTION_DOWN: {
			...
			Log.e("#######################",mCurlState+":"+mCurrentIndex);
			
			if (mCurlState == CURL_NONE) {
				if(onPageAnimationDoneListener!=null){
					onPageAnimationDoneListener.onPageNoAnimation(mCurrentIndex);
				}
				return false;
			}
		}
		...
		case MotionEvent.ACTION_UP: {
			...
			if (mCurlState == CURL_LEFT || mCurlState == CURL_RIGHT) {
				...
				if ((mViewMode == SHOW_ONE_PAGE && mPointerPos.mPos.x > (rightRect.left + rightRect.right) / 2)
					...
					Log.e("OOOOOOOOOOOOOO", mCurlState+",,,,,,,");
					if(mCurlState == CURL_LEFT){//翻页成功
						if(onPageAnimationDoneListener!=null){
							onPageAnimationDoneListener.onBeforeAnimationDone((mCurrentIndex-1));
						}
					}
				} else {
					...
					Log.e("OOOOOOOOOOOOOO", mCurlState+"ppppppppp");
					if(mCurlState == CURL_RIGHT){//翻页成功
						if(onPageAnimationDoneListener!=null){
							onPageAnimationDoneListener.onBeforeAnimationDone((mCurrentIndex+1));
						}
					}
	...
	private void startCurl(int page) {
		...
		case CURL_RIGHT: {
			...
			if (mCurrentIndex > 0) {
				...
				mPageLeft.reset();
				Log.e("startCurl----", "startCurl");
				if (mRenderLeftPage) {
					Log.e("startCurl----", "addCurlMesh+mRenderLeftPage");
			...
			if (mCurrentIndex < mPageProvider.getPageCount() - 1) {
				...
				Log.e("startCurl----", "addCurlMesh+mPageRight");
			}
			...
			Log.e("startCurl----", "addCurlMesh+mPageCurl");
